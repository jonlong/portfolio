...
I could fill a book with the things I learned leading this project, but over time I continually return to what now feels like its singular lesson: while we had no idea what the path would look like, we knew it was possible to figure it out together.
...
Lag was a prime example. Livestreaming partners had to be lined up early in the project's lifecycle, and our team was assured that the platform's lag time would be a couple seconds at most. After designing the gameplay accordingly, we discovered a few weeks later during testing that the actual lag time was anywhere from five seconds to a full minute, instantly rendering weeks of work unusable.

With time bearing down we couldn't afford a full refactor of the game mechanics, so the team had 24 hours to design and test a new approach that could keep the project afloat. Huddled together, we devised a queue-based system, where players would position each of their shots using the existing interface, and trigger the cannon to fire them off in sequence. We padded the time between gameplay elements to keep any of the action from going too far out of sync with the video, and had a working prototype running the next day.
...
We split the app into 4 components, each of which could be replicated across multiple servers: one in charge of the realtime event handling, one in charge of site and session logic, one in charge of the database, one in charge of ephemeral global game state.
